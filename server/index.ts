...
app.post("/register", async (req, res) => {
  try {
    const { username, password, email } = req.body; // <--- Nhận thêm email
...
app.post("/login", async (req, res) => {
  try {
    ...
    const user = await UserModel.findOne({
      $or: [
        { username: username }, // Tìm theo tên đăng nhập
        { email: username }, // Tìm theo email
      ],
    });
...
app.post("/reset-password", async (req, res) => {
  try {
    ...
    // Tìm user có đúng Username VÀ Email này
...
app.post("/api/books", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Đã thêm sách vào hệ thống!", book: newBook });
  } catch (err) {
...
app.get("/api/my-books", async (req, res) => {
  try {
    ...
    const books = await PersonalBookModel.find({ userId: userId }).sort({
      createdAt: -1,
    });
  ...
  try {
    ...
    if (req.file.mimetype === "application/pdf") {
      ...
      if (typeof pdfParse !== "function") {
        throw new Error(
          `Vẫn không tìm thấy hàm! Type hiện tại là: ${typeof pdfParse}`,
        );
      }
...
app.get("/api/books/:id", async (req, res) => {
  try {
    ...
    if (!book) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy sách trong hệ thống" });
    }
    ...
    res
      .status(500)
      .json({ message: "Lỗi ID sách không hợp lệ: " + err.message });
  }
});
// // ==========================================
...
app.get("/api/admin/pending-books", async (req, res) => {
  try {
    const pendingBooks = await PersonalBookModel.find({
      status: "pending",
    }).sort({ createdAt: -1 });
...
app.put("/api/admin/approve/:bookId", async (req, res) => {
  try {
    await PersonalBookModel.findByIdAndUpdate(req.params.bookId, {
      status: "approved",
    });
